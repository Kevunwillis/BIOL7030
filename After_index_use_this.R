...
BiocManager::install("Rsamtools",force=T)
...
fl2 <- system("ls -d /standard/BerglandTeach/mapping_output/PRJNA185744/*.original.bam", intern=T)
...
ggplot(data=rd, aes(x=realProp, y=propSim)) + geom_point() +
  geom_hline(data=rd2, aes(yintercept=propSim, color=propSim))