...
> test_funHW <- function(p) {
  ...
  +   pq <- 2*p*(1-p)
  +   
    +   return(data.table(p=p, pp=pp, pq=pq, qq=qq))
  }
out2 <- test_funHW(p=c(1:100)/100)
out3 <- melt(out2, id.vars = "p", measure.vars = c("pp","pq","qq"))
ggplot(data=out3) + geom_line(aes(x=p, y = value, group = variable, colour = variable))